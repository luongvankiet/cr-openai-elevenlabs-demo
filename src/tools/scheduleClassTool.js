/**
 * Schedule Class Tool
 * Allows the AI to help students reschedule their classes
 */

const scheduleClassTool = {
  name: "schedule_class",
  description: "Help student reschedule their class to a different date or time",
  parameters: {
    type: "object",
    properties: {
      action: {
        ...
        enum: ["reschedule", "cancel", "confirm_current"]
      },
      newDate: {
        type: "string",
      ...
      reason: {
        type: "string",
        description: "Reason for rescheduling (e.g., 'conflict', 'illness', 'emergency')"
      },
      studentPreference: {
        type: "string",
          ...
          if (newDate && newTime && currentStudent) {
            ...
            if (sheetUpdateResult.success) {
              ...
              if (sessionData.studentInfo) {
                ...
                sessionData.studentInfo.rescheduledCallAt = new Date().toISOString();
                sessionData.studentInfo.notes = `Rescheduled via phone call: ${reason || 'No reason provided'}`;
              }
            } else {
            ...
            availableSlots = await this.getAvailableTimeSlots(currentStudent?.className);
            responseMessage = `I can help you reschedule your class. Here are some available options: ${availableSlots.join(', ')}. Which date and time works best for you?`;
          }
          break;
          ...
          if (currentStudent) {
            ...
            if (sheetUpdateResult.success) {
              ...
              if (sessionData.studentInfo) {
                ...
                sessionData.studentInfo.reason = reason || "Cancelled via phone call";
                sessionData.studentInfo.notes = `Cancelled via phone call: ${reason || 'No reason provided'}`;
              }
            } else {
              responseMessage = "I've noted your cancellation request. Someone from our team will follow up with you about the cancellation and any makeup options.";
            }
          } else {
            responseMessage = "I've noted your cancellation request and someone will follow up with you soon.";
          }
          break;
          ...
          if (currentStudent) {
            ...
            if (sheetUpdateResult.success) {
              ...
              if (sessionData.studentInfo) {
                ...
                sessionData.studentInfo.notes = "Attendance confirmed via phone call";
              }
            } else {
              responseMessage = "Thanks for confirming! We look forward to seeing you in class. If you have any questions, feel free to reach out.";
            }
          } else {
            responseMessage = "Thank you for confirming your attendance! We look forward to seeing you in class.";
          }
          break;

        default:
          responseMessage = "I've noted your scheduling request and someone will follow up with you soon.";
      }
    } catch (error) {
      console.error(`Error in schedule tool action ${action}:`, error);
      responseMessage = "I've noted your request, but couldn't update the system right now. Someone from our team will follow up with you shortly.";
    }
   ...
   */
  async updateStudentScheduleInSheets(student, newDate, newTime, reason, googleSheetsService) {
    try {
      ...
      const updateData = {
        ...
        'Notes': `Rescheduled via phone call on ${new Date().toLocaleDateString()}: ${reason || 'No reason provided'}`
      };
      ...
      console.log(`Successfully updated student ${student.name} schedule in Google Sheets`);
  ...
  async cancelStudentClassInSheets(student, reason, googleSheetsService) {
    try {
      ...
      const updateData = {
        ...
        'Notes': `Cancelled via phone call on ${new Date().toLocaleDateString()}: ${reason || 'No reason provided'}`
      };

      // Update the sheet
      const result = await googleSheetsService.updateRow(studentRowIndex + 2, updateData);
      
      console.log(`Successfully cancelled student ${student.name} class in Google Sheets`);
      return { success: true, result };
  ...
  async confirmStudentAttendanceInSheets(student, googleSheetsService) {
    try {
      ...
      console.log(`Successfully confirmed student ${student.name} attendance in Google Sheets`);
      return { success: true, result };

    } catch (error) {
      console.error("Error confirming student attendance in Google Sheets:", error);
      return { success: false, error: error.message };
    }
  },
